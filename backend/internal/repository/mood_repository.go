...
type MoodRepository interface {
	...
	Range(uid uint, start, end time.Time) ([]model.Mood, error)
...
func (r *moodRepository) Range(uid uint, start, end time.Time) (out []model.Mood, e error) {
	e = r.db.Where("user_id = ? AND record_date >= ? AND record_date < ?", uid, start, end).Order("record_date desc, id desc").Find(&out).Error
	return
}
